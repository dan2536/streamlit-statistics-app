...
st.write("두 변수(X, Y)의 관계를 분석하고, 상관계수, 회귀식, 산점도를 보여줍니다.")
...
if analyze_button: # 버튼이 클릭되면 이 블록 실행
    ...
    if errors:
        ...
        if correlation_coefficient > 0:
            st.info("  - 양의 상관관계: 한 변수가 증가하면 다른 변수도 증가하는 경향")
        elif correlation_coefficient < 0:
            st.info("  - 음의 상관관계: 한 변수가 증가하면 다른 변수는 감소하는 경향")
        # 0인 경우는 위의 info 메시지로 충분
